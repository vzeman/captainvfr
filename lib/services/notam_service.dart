...
class NotamService {
  ...
  static const String _faaBaseUrl =
  ...
  Future<List<Notam>> getNotamsForAirport(
    ...
    developer.log(
      '📋 Fetching NOTAMs for $icaoCode (forceRefresh: $forceRefresh)',
    );
    ...
    if (!forceRefresh) {
      try {
        ...
        if (cachedNotams.isNotEmpty && _isCacheValid(cachedNotams)) {
          developer.log(
            '✅ Using cached NOTAMs for $icaoCode (${cachedNotams.length} items)',
          );
          return cachedNotams;
        }
      } catch (e) {
    ...
    try {
      ...
      try {
        ...
        if (cachedNotams.isNotEmpty) {
          developer.log(
            '⚠️ Using expired cache for $icaoCode due to API error',
          );
          return cachedNotams;
        }
      } catch (cacheError) {
  ...
  Future<List<Notam>> _fetchNotamsFromAPI(String icaoCode) async {
    ...
    final uri = Uri.parse(_faaBaseUrl).replace(
      queryParameters: {
        'dataSource': 'notams',
        ...
        'hoursBeforeNow':
            '168', // Get NOTAMs from last 7 days instead of 24 hours
      },
    );
  ...
  List<Notam> _parseNotamsXml(String xmlString, String icaoCode) {
    try {
      ...
      if (notamElements.isEmpty) {
        ...
        developer.log(
          '🔍 Found ${notamElements.length} notam elements (lowercase)',
        );
      }
      ...
      if (notamElements.isEmpty) {
        ...
        if (dataElements.isNotEmpty) {
          ...
          developer.log(
            '🔍 Found ${notamElements.length} NOTAM elements under data element',
          );
        }
      }
      ...
      for (final notamElement in notamElements) {
        try {
          ...
          final notamId =
              notamElement.findElements('notamID').firstOrNull?.innerText ??
              notamElement.findElements('notam_id').firstOrNull?.innerText ??
              notamElement.findElements('id').firstOrNull?.innerText ??
              '';

          final facilityId =
              notamElement.findElements('facilityID').firstOrNull?.innerText ??
              notamElement.findElements('facility_id').firstOrNull?.innerText ??
              notamElement.findElements('icaoId').firstOrNull?.innerText ??
              icaoCode;

          final startDate =
              notamElement.findElements('startDate').firstOrNull?.innerText ??
              notamElement.findElements('start_date').firstOrNull?.innerText ??
              notamElement
                  .findElements('effectiveStart')
                  .firstOrNull
                  ?.innerText;

          final endDate =
              notamElement.findElements('endDate').firstOrNull?.innerText ??
              notamElement.findElements('end_date').firstOrNull?.innerText ??
              notamElement.findElements('effectiveEnd').firstOrNull?.innerText;

          final icaoMessage =
              notamElement.findElements('icaoMessage').firstOrNull?.innerText ??
              notamElement
                  .findElements('icao_message')
                  .firstOrNull
                  ?.innerText ??
          ...
          final traditionalMessage =
              notamElement
                  .findElements('traditionalMessage')
                  .firstOrNull
                  ?.innerText ??
              notamElement
                  .findElements('traditional_message')
                  .firstOrNull
                  ?.innerText;

          final plainLanguageMessage =
              notamElement
                  .findElements('plainLanguageMessage')
                  .firstOrNull
                  ?.innerText ??
              notamElement
                  .findElements('plain_language_message')
                  .firstOrNull
                  ?.innerText ??
          ...
          if (classification?.toUpperCase().contains('NEW') == true) {
            type = 'N';
          } else if (classification?.toUpperCase().contains('REPLACE') ==
              true) {
            type = 'R';
          ...
          final displayText =
              plainLanguageMessage ?? traditionalMessage ?? icaoMessage;
