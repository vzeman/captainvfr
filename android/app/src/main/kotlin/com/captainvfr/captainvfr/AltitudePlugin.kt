...
fun registerWith(registrar: io.flutter.plugin.common.PluginRegistry.Registrar) {
    val channel = MethodChannel(registrar.messenger(), "altitude_service")
    val plugin = AltitudePlugin()
    
    // This is a workaround to initialize the plugin without the binding
    val field = plugin.javaClass.getDeclaredField("channel")
    ...
    // Set up event channel
    val eventChannel = EventChannel(registrar.messenger(), "altitude_service/updates")
    eventChannel.setStreamHandler(plugin)
    
    // Initialize sensor manager
    val sensorManager = registrar.context().getSystemService(Context.SENSOR_SERVICE) as? SensorManager
    val fieldSensorManager = plugin.javaClass.getDeclaredField("sensorManager")
    fieldSensorManager.isAccessible = true
    fieldSensorManager.set(plugin, sensorManager)
    
    val pressureSensor = sensorManager?.getDefaultSensor(Sensor.TYPE_PRESSURE)
    val fieldPressureSensor = plugin.javaClass.getDeclaredField("pressureSensor")
    fieldPressureSensor.isAccessible = true
    fieldPressureSensor.set(plugin, pressureSensor)
    
    // Create a background thread for sensor updates
    val sensorThread = HandlerThread("SensorThread").apply { start() }
    val fieldSensorThread = plugin.javaClass.getDeclaredField("sensorThread")
    fieldSensorThread.isAccessible = true
    fieldSensorThread.set(plugin, sensorThread)
    
    val sensorHandler = Handler(sensorThread.looper)
    val fieldSensorHandler = plugin.javaClass.getDeclaredField("sensorHandler")
    fieldSensorHandler.isAccessible = true
    fieldSensorHandler.set(plugin, sensorHandler)
}
